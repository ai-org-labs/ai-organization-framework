...
test("visibility view loader and HTML shell align with the v1.4 visibility contract", async (t) => {
  ...
  assert.match(html, /AOF ミッションコントロール/);
  assert.match(html, /現在のミッション/);
  assert.match(html, /リリース \/ フロンティア/);
  assert.match(html, /ランタイム証跡/);
  assert.match(html, /リスク\/ブロッカー/);
  assert.match(html, /次にやること/);
  assert.match(html, /タスク状況/);
  assert.match(html, /data-tab-scope="tasks"/);
  assert.match(html, /data-tab-target="task-kanban"/);
  assert.match(html, /data-tab-target="task-list"/);
  assert.match(html, /バックログ/);
  assert.match(html, /発見/);
  assert.match(html, /必要性検証/);
  assert.match(html, /計画/);
  assert.match(html, /役割作業/);
  assert.match(html, /評議会レビュー/);
  assert.match(html, /承認\/完了（最新）/);
  assert.match(html, /全体概要/);
  assert.match(html, /詳細ビュー/);
  assert.match(html, /data-tab-target="tab-org"/);
  assert.match(html, /data-tab-target="tab-product"/);
  assert.match(html, /data-tab-target="tab-tests"/);
  assert.match(html, /組織トポロジー/);
  assert.match(html, /プロダクト完成度/);
  assert.match(html, /テスト\/監査/);
  assert.match(html, /ワークガバナンスチェーン/);
  assert.match(html, /証跡 \/ Proof Coverage/);
