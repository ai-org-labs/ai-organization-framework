...
test("situationAssessCommand diagnoses the current frontier from self-hosting runtime state", async () => {
  ...
  assert.equal(result.summary.active_release_version, "11.5.0");
  ...
  assert.match(result.summary.recommended_action.recommended_action, /v11\.6|external operator|reproduction feedback|feedback intake/i);
...
test("roadmapStatusCommand keeps committed release evidence on the correct tracks", async () => {
  ...
  assert.match(result.alignment.answer, /v11\.6|external operator|reproduction feedback|feedback intake/i);
...
test("visibilityExportCommand surfaces situation judgment rather than stale release work", async () => {
  ...
  assert.equal(result.ok, true);
  assert.equal(result.payloads.mission_control.mission_overview.release_version, "11.5.0");
  assert.equal(result.payloads.mission_control.mission_overview.current_runtime_stage, "frontier-definition-needed");
  assert.match(result.payloads.mission_control.next_action.recommended_action, /v11\.6|external operator|reproduction feedback|feedback intake/i);
  ...
  assert.match(result.payloads.operator_brief.headline, /frontier|v11\.6|external operator|reproduction feedback|feedback intake/i);
  assert.match(result.payloads.operator_brief.next_action.recommended_action, /v11\.6|external operator|reproduction feedback|feedback intake/i);
  ...
  assert.equal(result.payloads.mission_control.archmap.latest_work_item_id, "TASK-136");
  ...
  assert.equal(result.payloads.mission_control.agent_session_observability.latest_session_id, "SESS-V115-PROVIDER-OBSERVATION-REPLAY");
  assert.equal(result.payloads.mission_control.agent_session_observability.audit_ok, true);
  assert.ok(result.payloads.mission_control.agent_session_observability.linked_task_refs.some((ref) => /TASK-136/.test(ref)));
...
test("operatorBriefCommand compresses runtime situation judgment into one operator-facing packet", async () => {
  ...
  assert.equal(result.brief.current_state.release_version, "11.5.0");
  ...
  assert.match(result.brief.operator_answers.what_should_happen_next, /v11\.6|external operator|reproduction feedback|feedback intake/i);
});
...
test("organizationStatusCommand exposes the post-v10.1 direction goal and next value slice", async () => {
  ...
  assert.equal(result.ok, true);
  assert.match(result.goals.operating_goal, /v11\.6|external operator|reproduction feedback|feedback intake/i);
  assert.match(result.goals.next_value_slice, /v11\.6|external operator|reproduction feedback|feedback intake/i);
});
...
test("treePositionCommand explains the current release trunk and frontier branch", async () => {
  ...
  assert.equal(result.tree.trunk.active_release_version, "11.5.0");
  assert.equal(result.tree.branch.frontier_task_id, null);
  assert.equal(result.tree.branch.frontier_track, null);
  assert.match(result.tree.tree_answer.where_are_we, /between v11\.5 and the next concrete branch/i);
});
...
test("releaseStateAuditCommand includes product value, production boundary, and controlled candidate release gates", async () => {
  ...
  assert.equal(result.ok, true);
  assert.equal(result.summary.active_release.release_version, "11.5.0");
