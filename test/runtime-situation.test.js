...
test("situationAssessCommand diagnoses the current frontier from self-hosting runtime state", async () => {
  ...
  assert.match(result.summary.recommended_action.recommended_action, /runtime-backed direction review|v6\.8|frontier/i);
...
test("roadmapStatusCommand keeps completed v5/v6 release work on the correct tracks", async () => {
  ...
  assert.match(result.alignment.answer, /runtime-backed direction review|v6\.8|frontier/i);
...
test("visibilityExportCommand surfaces situation judgment rather than stale release work", async () => {
  ...
  assert.match(result.payloads.mission_control.next_action.recommended_action, /runtime-backed direction review|v6\.8|frontier/i);
  ...
  assert.match(result.payloads.operator_brief.headline, /runtime-backed direction review|v6\.8|frontier/i);
  assert.match(result.payloads.operator_brief.next_action.recommended_action, /runtime-backed direction review|v6\.8|frontier/i);
  ...
  assert.equal(result.payloads.mission_control.archmap.latest_work_item_id, "TASK-076");
...
test("operatorBriefCommand compresses runtime situation judgment into one operator-facing packet", async () => {
  ...
  assert.match(result.brief.operator_answers.what_should_happen_next, /runtime-backed direction review|v6\.8|frontier/i);
});
...
test("organizationStatusCommand exposes the post-v6.0 direction goal and next value slice", async () => {
  ...
  assert.match(result.goals.next_value_slice, /runtime-backed direction review|v6\.8|QIF boundary/i);
});
...
test("operatorProgressCommand explains what changed since the last checkpoint", async () => {
  ...
  assert.match(result.progress.progress_answer.what_changed, /TASK-076|direction review|v6\.7|v6\.8/i);
});
...
test("treePositionCommand explains the current release trunk and frontier branch", async () => {
  ...
  assert.match(result.tree.tree_answer.where_are_we, /v6\.7|v6\.8|frontier/i);
});
