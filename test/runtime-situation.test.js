...
test("situationAssessCommand diagnoses the current frontier from self-hosting runtime state", async () => {
  ...
  assert.equal(result.summary.active_release_version, "8.6.0");
  ...
  assert.match(result.summary.recommended_action.recommended_action, /v8\.7|External Runtime Reproduction and Rollback Proof|frontier/i);
...
test("roadmapStatusCommand keeps committed release evidence on the correct tracks", async () => {
  ...
  assert.match(result.alignment.answer, /v8\.7|External Runtime Reproduction and Rollback Proof|frontier/i);
...
test("visibilityExportCommand surfaces situation judgment rather than stale release work", async () => {
  ...
  assert.equal(result.ok, true);
  assert.equal(result.payloads.mission_control.mission_overview.release_version, "8.6.0");
  assert.equal(result.payloads.mission_control.mission_overview.current_runtime_stage, "frontier-definition-needed");
  assert.match(result.payloads.mission_control.next_action.recommended_action, /v8\.7|External Runtime Reproduction and Rollback Proof|frontier/i);
  ...
  assert.match(result.payloads.operator_brief.headline, /frontier|implementation task|v8\.7|External Runtime Reproduction and Rollback Proof/i);
  assert.match(result.payloads.operator_brief.next_action.recommended_action, /v8\.7|External Runtime Reproduction and Rollback Proof|frontier/i);
  ...
  assert.equal(result.payloads.mission_control.archmap.latest_work_item_id, "TASK-106");
  ...
  assert.equal(result.payloads.mission_control.agent_session_observability.latest_session_id, "SESS-V86-PROVIDER-EXECUTION-APPROVAL");
  assert.equal(result.payloads.mission_control.agent_session_observability.audit_ok, true);
  assert.ok(result.payloads.mission_control.agent_session_observability.linked_task_refs.some((ref) => /TASK-106/.test(ref)));
  ...
  assert.equal(result.payloads.mission_control.provider_adapter_pilot_projection.pilot_count, 1);
  ...
  assert.equal(result.payloads.mission_control.provider_execution_approval_projection.approval_count, 1);
  ...
  assert.equal(result.payloads.mission_control.external_runtime_safety_projection.safety_status, "governance_boundary_ready");
...
test("operatorBriefCommand compresses runtime situation judgment into one operator-facing packet", async () => {
  ...
  assert.equal(result.brief.current_state.release_version, "8.6.0");
  ...
  assert.match(result.brief.operator_answers.what_should_happen_next, /v8\.7|External Runtime Reproduction and Rollback Proof|frontier/i);
});

test("organizationStatusCommand exposes the post-v8.6 direction goal and next value slice", async () => {
  const projectRoot = process.cwd();
  const result = await organizationStatusCommand({ project: projectRoot });

  assert.equal(result.ok, true);
  assert.match(result.goals.operating_goal, /v8\.7|External Runtime Reproduction and Rollback Proof/i);
  assert.match(result.goals.next_value_slice, /v8\.7|runtime-backed direction review|External Runtime Reproduction and Rollback Proof/i);
});
...
test("operatorProgressCommand explains what changed since the last checkpoint", async () => {
  ...
  assert.match(result.progress.progress_answer.what_changed, /TASK-106|v8\.6|Provider Execution Approval Bridge|v8\.7/i);
});
...
test("treePositionCommand explains the current release trunk and frontier branch", async () => {
  ...
  assert.equal(result.tree.trunk.active_release_version, "8.6.0");
  assert.equal(result.tree.branch.frontier_task_id, null);
  assert.equal(result.tree.branch.frontier_track, null);
  assert.match(result.tree.tree_answer.where_are_we, /between v8\.6 and the next concrete branch|v8\.6\.0|v8\.7/i);
});

test("releaseStateAuditCommand includes the v8.6 provider execution approval release gate", async () => {
  const projectRoot = process.cwd();
  const result = await releaseStateAuditCommand({ project: projectRoot });

  assert.equal(result.ok, true);
  assert.equal(result.summary.active_release.release_version, "8.6.0");
