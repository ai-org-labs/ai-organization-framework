...
test("situationAssessCommand diagnoses the current frontier from self-hosting runtime state", async () => {
  ...
  assert.equal(result.summary.primary_frontier_task.task_id, "TASK-076");
  assert.equal(result.summary.current_runtime_stage, "implementation-ready");
  assert.match(result.summary.recommended_action.recommended_action, /TASK-076|release closure|v6\.7/i);
  assert.deepEqual(result.summary.operator_alignment.prioritized_task_ids, ["TASK-076"]);
...
test("roadmapStatusCommand keeps completed v5/v6 release work on the correct tracks", async () => {
  ...
  assert.equal(result.ok, true);
  assert.deepEqual(result.alignment.prioritized_task_ids, ["TASK-076"]);
  assert.match(result.alignment.answer, /TASK-076|release closure|v6\.7/i);
...
test("visibilityExportCommand surfaces situation judgment rather than stale release work", async () => {
  ...
  assert.match(result.payloads.mission_control.next_action.recommended_action, /TASK-076|release closure|v6\.7/i);
  ...
  assert.match(result.payloads.operator_brief.headline, /TASK-076|v6\.7|frontier/i);
  assert.match(result.payloads.operator_brief.next_action.recommended_action, /TASK-076|release closure|v6\.7/i);
...
test("operatorBriefCommand compresses runtime situation judgment into one operator-facing packet", async () => {
  ...
  assert.equal(result.brief.current_state.primary_frontier_task.task_id, "TASK-076");
  assert.equal(result.brief.current_state.skillful_actor_projection?.projection_id, "SAHRI-TASK-054-PROOF");
  assert.match(result.brief.operator_answers.what_should_happen_next, /TASK-076|release closure|v6\.7/i);
});
...
test("organizationStatusCommand exposes the post-v6.0 direction goal and next value slice", async () => {
  ...
  assert.match(result.goals.next_value_slice, /TASK-076|release closure|QIF boundary/i);
});
...
test("operatorProgressCommand explains what changed since the last checkpoint", async () => {
  ...
  assert.match(result.progress.progress_answer.what_changed, /TASK-075|TASK-076|release closure|v6\.7/i);
});
...
test("treePositionCommand explains the current release trunk and frontier branch", async () => {
  ...
  assert.equal(result.tree.branch.frontier_task_id, "TASK-076");
  assert.equal(result.tree.branch.frontier_track, "v6.7");
  assert.match(result.tree.tree_answer.where_are_we, /v6\.6|v6\.7|TASK-076/i);
});
