...
test("situationAssessCommand diagnoses the current frontier from self-hosting runtime state", async () => {
  ...
  assert.equal(result.summary.active_release_version, "12.0.0");
  ...
  assert.match(result.summary.recommended_action.recommended_action, /v12\.1|external operator|decision console|reproduce|Go\/No-Go/i);
  ...
  assert.equal(
    result.summary.current_truth_conflicts.every((conflict) =>
      ["shipped-release-task-open", "frontier-task-mismatch"].includes(conflict.code)
    ),
    true
  );
});
...
test("roadmapStatusCommand keeps committed release evidence on the correct tracks", async () => {
  ...
  assert.match(result.alignment.answer, /v12\.1|external operator|decision console|reproduce|Go\/No-Go/i);
  ...
  assert.ok(Array.isArray(result.release_tracks["v8.3"]));
  assert.ok(result.release_tracks["v8.3"].some((task) => task.task_id === "TASK-103"));
  assert.ok(Array.isArray(result.release_tracks["v8.4"]));
  assert.ok(result.release_tracks["v8.4"].some((task) => task.task_id === "TASK-104"));
  assert.ok(Array.isArray(result.release_tracks["v8.5"]));
  assert.ok(result.release_tracks["v8.5"].some((task) => task.task_id === "TASK-105"));
  assert.ok(Array.isArray(result.release_tracks["v8.6"]));
  ...
  assert.ok(Array.isArray(result.release_tracks["v9.0"]));
  assert.ok(result.release_tracks["v9.0"].some((task) => task.task_id === "TASK-110"));
  assert.ok(Array.isArray(result.release_tracks["v9.1"]));
  assert.ok(result.release_tracks["v9.1"].some((task) => task.task_id === "TASK-111"));
  assert.ok(Array.isArray(result.release_tracks["v9.2"]));
  assert.ok(result.release_tracks["v9.2"].some((task) => task.task_id === "TASK-112"));
  assert.ok(Array.isArray(result.release_tracks["v9.3"]));
  assert.ok(result.release_tracks["v9.3"].some((task) => task.task_id === "TASK-113"));
  assert.ok(Array.isArray(result.release_tracks["v9.4"]));
  assert.ok(result.release_tracks["v9.4"].some((task) => task.task_id === "TASK-114"));
  assert.ok(Array.isArray(result.release_tracks["v9.5"]));
  assert.ok(result.release_tracks["v9.5"].some((task) => task.task_id === "TASK-115"));
  assert.ok(Array.isArray(result.release_tracks["v9.6"]));
  assert.ok(result.release_tracks["v9.6"].some((task) => task.task_id === "TASK-116"));
  assert.ok(Array.isArray(result.release_tracks["v9.7"]));
  assert.ok(result.release_tracks["v9.7"].some((task) => task.task_id === "TASK-117"));
  assert.ok(Array.isArray(result.release_tracks["v9.8"]));
  assert.ok(result.release_tracks["v9.8"].some((task) => task.task_id === "TASK-118"));
  assert.ok(Array.isArray(result.release_tracks["v10.8"]));
  assert.ok(result.release_tracks["v10.8"].some((task) => task.task_id === "TASK-129"));
  assert.ok(Array.isArray(result.release_tracks["v10.9"]));
  assert.ok(result.release_tracks["v10.9"].some((task) => task.task_id === "TASK-130"));
  assert.ok(Array.isArray(result.release_tracks["v11.0"]));
  assert.ok(result.release_tracks["v11.0"].some((task) => task.task_id === "TASK-131"));
  assert.ok(Array.isArray(result.release_tracks["v11.1"]));
  assert.ok(result.release_tracks["v11.1"].some((task) => task.task_id === "TASK-132"));
  assert.ok(Array.isArray(result.release_tracks["v11.2"]));
  assert.ok(result.release_tracks["v11.2"].some((task) => task.task_id === "TASK-133"));
  assert.ok(Array.isArray(result.release_tracks["v11.3"]));
  assert.ok(result.release_tracks["v11.3"].some((task) => task.task_id === "TASK-134"));
  assert.ok(Array.isArray(result.release_tracks["v11.4"]));
  ...
  assert.ok(Array.isArray(result.release_tracks["v11.8"]));
  assert.ok(result.release_tracks["v11.8"].some((task) => task.task_id === "TASK-139"));
  assert.ok(Array.isArray(result.release_tracks["v11.9"]));
  assert.ok(result.release_tracks["v11.9"].some((task) => task.task_id === "TASK-141"));
  assert.ok(Array.isArray(result.release_tracks["v12.0"]));
  assert.ok(result.release_tracks["v12.0"].some((task) => task.task_id === "TASK-142"));
});
...
test("visibilityExportCommand surfaces situation judgment rather than stale release work", async () => {
  ...
  assert.equal(result.ok, true);
  assert.equal(result.payloads.mission_control.mission_overview.release_version, "12.0.0");
  assert.equal(result.payloads.mission_control.mission_overview.current_runtime_stage, "frontier-definition-needed");
  assert.match(result.payloads.mission_control.next_action.recommended_action, /v12\.1|external operator|decision console|reproduce|Go\/No-Go/i);
  ...
  assert.match(result.payloads.operator_brief.headline, /frontier|v12\.0|v12\.1|decision console|provider-backed|external operator/i);
  assert.match(result.payloads.operator_brief.next_action.recommended_action, /v12\.1|external operator|decision console|reproduce|Go\/No-Go/i);
  ...
  assert.equal(result.payloads.mission_control.archmap.latest_work_item_id, "TASK-142");
  ...
  assert.equal(result.payloads.mission_control.agent_session_observability.latest_session_id, "SESS-V120-OPERATOR-DECISION-CONSOLE");
  assert.equal(result.payloads.mission_control.agent_session_observability.audit_ok, true);
  assert.ok(result.payloads.mission_control.agent_session_observability.linked_task_refs.some((ref) => /TASK-142/.test(ref)));
  ...
  assert.equal(result.payloads.mission_control.tool_governance_replay.present, true);
  assert.equal(result.payloads.mission_control.tool_governance_replay.source_session_id, "SESS-V120-OPERATOR-DECISION-CONSOLE");
  assert.equal(result.payloads.mission_control.tool_governance_replay.tool_call_count, 2);
  assert.equal(result.payloads.mission_control.tool_governance_replay.governed_tool_call_count, 2);
  assert.equal(result.payloads.mission_control.tool_governance_replay.allowed_count, 2);
  assert.equal(result.payloads.mission_control.tool_governance_replay.denied_count, 0);
  assert.equal(result.payloads.mission_control.tool_governance_replay.review_required_count, 0);
  assert.equal(result.payloads.mission_control.tool_governance_replay.risky_count, 0);
  assert.equal(result.payloads.mission_control.tool_governance_replay.external_write_count, 0);
  assert.equal(result.payloads.mission_control.tool_governance_replay.governance_status, "pass");
  assert.equal(result.payloads.mission_control.tool_governance_replay.replay_items[0].governance_decision, "allowed");
  assert.equal(result.payloads.mission_control.tool_governance_replay.replay_items[0].risk_class, "safe");
  ...
  assert.equal(result.payloads.mission_control.provider_adapter_pilot_projection.present, true);
  assert.equal(result.payloads.mission_control.provider_adapter_pilot_projection.audit_ok, true);
  assert.equal(result.payloads.mission_control.provider_adapter_pilot_projection.pilot_count, 2);
  assert.equal(result.payloads.mission_control.provider_execution_approval_projection.present, true);
  assert.equal(result.payloads.mission_control.provider_execution_approval_projection.audit_ok, true);
  assert.equal(result.payloads.mission_control.provider_execution_approval_projection.approval_count, 2);
  assert.equal(result.payloads.mission_control.provider_execution_approval_projection.approved_count, 1);
  assert.equal(result.payloads.mission_control.provider_execution_approval_projection.external_write_authorized_count, 1);
  assert.equal(result.payloads.mission_control.provider_execution_approval_projection.production_executed_count, 0);
  assert.equal(result.payloads.mission_control.provider_execution_reproduction_projection.present, true);
  assert.equal(result.payloads.mission_control.provider_execution_reproduction_projection.audit_ok, true);
  assert.equal(result.payloads.mission_control.provider_execution_reproduction_projection.reproduction_count, 1);
  assert.equal(result.payloads.mission_control.provider_execution_reproduction_projection.reproduced_count, 1);
  assert.equal(result.payloads.mission_control.provider_rollback_proof_projection.present, true);
  ...
  assert.equal(result.payloads.mission_control.provider_incident_recovery_projection.present, true);
  assert.equal(result.payloads.mission_control.provider_incident_recovery_projection.audit_ok, true);
  assert.equal(result.payloads.mission_control.provider_incident_recovery_projection.recovery_count, 1);
  assert.equal(result.payloads.mission_control.provider_incident_recovery_projection.ready_count, 1);
  assert.equal(result.payloads.mission_control.provider_cost_quota_boundary_projection.present, true);
  assert.equal(result.payloads.mission_control.provider_cost_quota_boundary_projection.audit_ok, true);
  assert.equal(result.payloads.mission_control.provider_cost_quota_boundary_projection.boundary_count, 1);
  assert.equal(result.payloads.mission_control.provider_cost_quota_boundary_projection.ready_count, 1);
  assert.equal(result.payloads.mission_control.provider_cost_quota_boundary_projection.blocked_count, 1);
  assert.equal(result.payloads.mission_control.provider_cost_quota_boundary_projection.allowed_count, 0);
  assert.equal(result.payloads.mission_control.operator_acceptance_drill_projection.present, true);
  assert.equal(result.payloads.mission_control.operator_acceptance_drill_projection.audit_ok, true);
  assert.equal(result.payloads.mission_control.operator_acceptance_drill_projection.drill_count, 1);
  assert.equal(result.payloads.mission_control.operator_acceptance_drill_projection.accept_count, 1);
  assert.equal(result.payloads.mission_control.product_value_evidence_projection.present, true);
  assert.equal(result.payloads.mission_control.product_value_evidence_projection.audit_ok, true);
  assert.ok(result.payloads.mission_control.product_value_evidence_projection.record_count >= 7);
  assert.ok(result.payloads.mission_control.product_value_evidence_projection.understood_count >= 7);
  assert.ok(result.payloads.mission_control.product_value_evidence_projection.capability_row_count >= 10);
  assert.equal(result.payloads.mission_control.provider_production_boundary_projection.present, true);
  assert.equal(result.payloads.mission_control.provider_production_boundary_projection.audit_ok, true);
  assert.equal(result.payloads.mission_control.provider_production_boundary_projection.boundary_count, 1);
  assert.equal(result.payloads.mission_control.provider_production_boundary_projection.production_authorized_count, 0);
  ...
  assert.equal(result.payloads.mission_control.external_runtime_safety_projection.safety_status, "blocked");
  assert.equal(result.payloads.mission_control.external_runtime_safety_projection.governance_boundary_status, "governance_boundary_ready");
  assert.equal(result.payloads.mission_control.external_runtime_safety_projection.production_execution_safety_status, "not_proven");
  assert.equal(result.payloads.mission_control.external_runtime_safety_projection.governance_action, "operator-review-allowed");
  assert.match(
    result.payloads.mission_control.external_runtime_safety_projection.not_proven,
    /not permission for autonomous external writes/i
  );
  assert.ok(result.payloads.mission_control.external_runtime_safety_projection.evidence_refs.some((ref) => /external-resource-audit/.test(ref)));
  assert.ok(result.payloads.mission_control.external_runtime_safety_projection.checks.some((check) => check.check_id === "external-resource-use"));
  assert.ok(result.payloads.mission_control.external_runtime_safety_projection.checks.some((check) => check.check_id === "provider-execution-approval-bridge"));
  ...
  assert.equal(result.payloads.mission_control.provider_read_freshness_refresh_projection.present, true);
  assert.equal(result.payloads.mission_control.provider_read_freshness_refresh_projection.audit_ok, true);
  assert.equal(result.payloads.mission_control.provider_read_freshness_refresh_projection.refresh_count, 1);
  assert.equal(result.payloads.mission_control.provider_read_freshness_refresh_projection.current_count, 1);
  assert.equal(result.payloads.mission_control.provider_read_freshness_refresh_projection.latest_freshness_status, "current");
  assert.equal(result.payloads.mission_control.provider_read_freshness_refresh_projection.latest_refresh_decision, "use_as_current");
  assert.equal(result.payloads.mission_control.provider_backed_operator_decision_console.present, true);
  assert.equal(result.payloads.mission_control.provider_backed_operator_decision_console.decision_state, "accepted");
  assert.equal(result.payloads.mission_control.provider_backed_operator_decision_console.freshness_status, "current");
  assert.equal(result.payloads.mission_control.provider_backed_operator_decision_console.tool_governance_status, "pass");
  assert.equal(result.payloads.mission_control.provider_backed_operator_decision_console.recommended_go_no_go, "block_provider_execution");
  assert.match(result.payloads.mission_control.provider_backed_operator_decision_console.no_autonomous_write_boundary, /must not execute provider writes/i);
  assert.ok(result.payloads.mission_control.provider_backed_operator_decision_console.evidence_refs.some((ref) => /SESS-V120/.test(ref)));
...
test("operatorBriefCommand compresses runtime situation judgment into one operator-facing packet", async () => {
  ...
  assert.equal(result.brief.current_state.release_version, "12.0.0");
  ...
  assert.match(result.brief.operator_answers.what_should_happen_next, /v12\.1|external operator|decision console|reproduce|Go\/No-Go/i);
});
...
test("organizationStatusCommand exposes the post-v10.1 direction goal and next value slice", async () => {
  ...
  assert.equal(result.ok, true);
  assert.match(result.goals.operating_goal, /v12\.1|external operator|decision console|reproduce/i);
  assert.match(result.goals.next_value_slice, /v12\.1|external operator|decision console|Go\/No-Go/i);
});
...
test("operatorProgressCommand explains what changed since the last checkpoint", async () => {
  ...
  assert.match(result.progress.progress_answer.what_changed, /TASK-142|v12\.0|operator decision|decision console/i);
});
...
test("treePositionCommand explains the current release trunk and frontier branch", async () => {
  ...
  assert.equal(result.tree.trunk.active_release_version, "12.0.0");
  assert.equal(result.tree.branch.frontier_task_id, null);
  assert.equal(result.tree.branch.frontier_track, null);
  assert.match(result.tree.tree_answer.where_are_we, /between v12\.0 and the next concrete branch/i);
});
...
test("releaseStateAuditCommand includes product value, production boundary, and controlled candidate release gates", async () => {
  ...
  assert.equal(result.ok, true);
  assert.equal(result.summary.active_release.release_version, "12.0.0");
  ...
  const providerAdapterPilotAudit = result.summary.governance_audits.find((audit) => audit.name === "provider-adapter-pilot-audit");
  ...
  const operatorAcceptanceDrillAudit = result.summary.governance_audits.find((audit) => audit.name === "operator-acceptance-drill-audit");
  assert.equal(operatorAcceptanceDrillAudit.ok, true);
  const productValueEvidenceAudit = result.summary.governance_audits.find((audit) => audit.name === "product-value-evidence-audit");
  assert.equal(productValueEvidenceAudit.ok, true);
  const providerProductionBoundaryAudit = result.summary.governance_audits.find((audit) => audit.name === "provider-production-boundary-audit");
  assert.equal(providerProductionBoundaryAudit.ok, true);
  const capabilityFirstReleaseAudit = result.summary.governance_audits.find((audit) => audit.name === "capability-first-release-audit");
  assert.equal(capabilityFirstReleaseAudit.ok, true);
  const capabilityCoverageAudit = result.summary.governance_audits.find((audit) => audit.name === "capability-coverage-audit");
  assert.equal(capabilityCoverageAudit.ok, true);
  const providerControlledExecutionCandidateAudit = result.summary.governance_audits.find((audit) => audit.name === "provider-controlled-execution-candidate-audit");
  assert.equal(providerControlledExecutionCandidateAudit.ok, true);
  const providerIncidentRecoveryAudit = result.summary.governance_audits.find((audit) => audit.name === "provider-incident-recovery-audit");
  assert.equal(providerIncidentRecoveryAudit.ok, true);
  const providerCostQuotaBoundaryAudit = result.summary.governance_audits.find((audit) => audit.name === "provider-cost-quota-boundary-audit");
  ...
  const providerReadFreshnessRefreshAudit = result.summary.governance_audits.find((audit) => audit.name === "provider-read-freshness-refresh-audit");
  assert.equal(providerReadFreshnessRefreshAudit.ok, true);
