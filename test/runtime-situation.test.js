...
test("situationAssessCommand diagnoses the current frontier from self-hosting runtime state", async () => {
  ...
  assert.equal(result.summary.active_release_version, "7.1.0");
  ...
  assert.match(result.summary.recommended_action.recommended_action, /v7\.2|Work Execution Packet|execution packet|frontier/i);
...
test("roadmapStatusCommand keeps completed v5/v6 release work on the correct tracks", async () => {
  ...
  assert.match(result.alignment.answer, /v7\.2|Work Execution Packet|execution packet|frontier/i);
...
test("visibilityExportCommand surfaces situation judgment rather than stale release work", async () => {
  ...
  assert.equal(result.ok, true);
  assert.equal(result.payloads.mission_control.mission_overview.release_version, "7.1.0");
  assert.equal(result.payloads.mission_control.mission_overview.current_runtime_stage, "frontier-definition-needed");
  assert.match(result.payloads.mission_control.next_action.recommended_action, /v7\.2|Work Execution Packet|execution packet|frontier/i);
  ...
  assert.match(result.payloads.operator_brief.headline, /frontier|implementation task|v7\.2|Work Execution Packet|execution packet/i);
  assert.match(result.payloads.operator_brief.next_action.recommended_action, /v7\.2|Work Execution Packet|execution packet|frontier/i);
  ...
  assert.equal(result.payloads.mission_control.archmap.latest_work_item_id, "TASK-090");
  ...
  assert.equal(result.payloads.mission_control.agent_session_observability.latest_session_id, "SESS-V71-CONTEXT-REFERENCE-INTEGRITY");
  assert.equal(result.payloads.mission_control.agent_session_observability.audit_ok, true);
  assert.ok(result.payloads.mission_control.agent_session_observability.linked_task_refs.some((ref) => /TASK-090/.test(ref)));
...
test("operatorBriefCommand compresses runtime situation judgment into one operator-facing packet", async () => {
  ...
  assert.equal(result.brief.current_state.release_version, "7.1.0");
  ...
  assert.match(result.brief.operator_answers.what_should_happen_next, /v7\.2|Work Execution Packet|execution packet|frontier/i);
});

test("organizationStatusCommand exposes the post-v7.1 direction goal and next value slice", async () => {
  const projectRoot = process.cwd();
  const result = await organizationStatusCommand({ project: projectRoot });

  assert.equal(result.ok, true);
  assert.match(result.goals.operating_goal, /v7\.2|Work Execution Packet|context\/reference integrity/i);
  assert.match(result.goals.next_value_slice, /v7\.2|Work Execution Packet|actor handoff|stop\/continue/i);
});
...
test("operatorProgressCommand explains what changed since the last checkpoint", async () => {
  ...
  assert.match(result.progress.progress_answer.what_changed, /TASK-090|v7\.1|context|reference/i);
});
...
test("treePositionCommand explains the current release trunk and frontier branch", async () => {
  ...
  assert.equal(result.tree.trunk.active_release_version, "7.1.0");
  assert.equal(result.tree.branch.frontier_task_id, null);
  assert.equal(result.tree.branch.frontier_track, null);
  assert.match(result.tree.tree_answer.where_are_we, /between v7\.1 and the next concrete branch/i);
});
