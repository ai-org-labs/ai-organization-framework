...
test("situationAssessCommand diagnoses the current frontier from self-hosting runtime state", async () => {
  ...
  assert.equal(result.summary.active_release_version, "6.5.0");
  assert.equal(result.summary.primary_frontier_task?.task_id, "TASK-071");
  assert.equal(result.summary.primary_frontier_task?.track, "v6.6");
  assert.equal(result.summary.current_runtime_stage, "implementation-ready");
  assert.match(result.summary.recommended_action.recommended_action, /TASK-071|Archmap|v6\.6/i);
  assert.deepEqual(result.summary.operator_alignment.prioritized_task_ids, ["TASK-071"]);
...
test("roadmapStatusCommand keeps completed v5/v6 release work on the correct tracks", async () => {
  ...
  assert.equal(result.ok, true);
  assert.deepEqual(result.alignment.prioritized_task_ids, ["TASK-071"]);
  assert.match(result.alignment.answer, /TASK-071|Archmap|v6\.6/i);
...
test("visibilityExportCommand surfaces situation judgment rather than stale release work", async () => {
  ...
  assert.equal(result.ok, true);
  assert.equal(result.payloads.mission_control.mission_overview.release_version, "6.5.0");
  assert.equal(result.payloads.mission_control.mission_overview.current_runtime_stage, "implementation-ready");
  assert.match(result.payloads.mission_control.next_action.recommended_action, /TASK-071|Archmap|v6\.6/i);
  ...
  assert.match(result.payloads.operator_brief.headline, /TASK-071|v6\.6/i);
  assert.match(result.payloads.operator_brief.next_action.recommended_action, /TASK-071|Archmap|v6\.6/i);
  ...
  assert.equal(result.payloads.mission_control.archmap.latest_work_item_id, "TASK-071");
  assert.equal(result.payloads.mission_control.archmap.pending_impact_count, 1);
...
test("operatorBriefCommand compresses runtime situation judgment into one operator-facing packet", async () => {
  ...
  assert.equal(result.brief.current_state.release_version, "6.5.0");
  assert.equal(result.brief.current_state.current_runtime_stage, "implementation-ready");
  assert.equal(result.brief.current_state.primary_frontier_task?.task_id, "TASK-071");
  assert.equal(result.brief.current_state.skillful_actor_projection?.projection_id, "SAHRI-TASK-054-PROOF");
  assert.match(result.brief.operator_answers.what_should_happen_next, /TASK-071|Archmap|v6\.6/i);
});
...
test("organizationStatusCommand exposes the post-v6.0 direction goal and next value slice", async () => {
  ...
  assert.equal(result.ok, true);
  assert.match(result.goals.operating_goal, /v6\.5|Execution Hygiene|Human Recognition/i);
  assert.match(result.goals.next_value_slice, /v6\.5|Execution Hygiene|runtime claims need executed command proof/i);
});
...
test("operatorProgressCommand explains what changed since the last checkpoint", async () => {
  ...
  assert.match(result.progress.progress_answer.what_changed, /TASK-070|Execution Hygiene|v6\.5/i);
});
...
test("treePositionCommand explains the current release trunk and frontier branch", async () => {
  ...
  assert.equal(result.tree.branch.frontier_task_id, "TASK-071");
  assert.equal(result.tree.branch.frontier_track, "v6.6");
});
