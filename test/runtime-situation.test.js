...
test("situationAssessCommand diagnoses the current frontier from self-hosting runtime state", async () => {
  ...
  assert.equal(result.summary.active_release_version, "11.2.0");
  ...
  assert.match(result.summary.recommended_action.recommended_action, /v11\.3|external validation|replay|provider-read confidence|uncertainty/i);
...
test("roadmapStatusCommand keeps committed release evidence on the correct tracks", async () => {
  ...
  assert.match(result.alignment.answer, /v11\.3|external validation|replay|provider-read confidence|uncertainty/i);
...
test("visibilityExportCommand surfaces situation judgment rather than stale release work", async () => {
  ...
  assert.equal(result.ok, true);
  assert.equal(result.payloads.mission_control.mission_overview.release_version, "11.2.0");
  assert.equal(result.payloads.mission_control.mission_overview.current_runtime_stage, "frontier-definition-needed");
  assert.match(result.payloads.mission_control.next_action.recommended_action, /v11\.3|external validation|replay|provider-read confidence|uncertainty/i);
  ...
  assert.match(result.payloads.operator_brief.headline, /frontier|v11\.3|external validation|replay|provider-read confidence|uncertainty/i);
  assert.match(result.payloads.operator_brief.next_action.recommended_action, /v11\.3|external validation|replay|provider-read confidence|uncertainty/i);
  ...
  assert.equal(result.payloads.mission_control.archmap.latest_work_item_id, "TASK-133");
...
test("operatorBriefCommand compresses runtime situation judgment into one operator-facing packet", async () => {
  ...
  assert.equal(result.brief.current_state.release_version, "11.2.0");
  ...
  assert.match(result.brief.operator_answers.what_should_happen_next, /v11\.3|external validation|replay|provider-read confidence|uncertainty/i);
});
...
test("organizationStatusCommand exposes the post-v10.1 direction goal and next value slice", async () => {
  ...
  assert.equal(result.ok, true);
  assert.match(result.goals.operating_goal, /v11\.3|external validation|replay|provider-read confidence|uncertainty/i);
  assert.match(result.goals.next_value_slice, /v11\.3|external validation|replay|provider-read confidence|uncertainty/i);
});
...
test("treePositionCommand explains the current release trunk and frontier branch", async () => {
  ...
  assert.equal(result.tree.trunk.active_release_version, "11.2.0");
  assert.equal(result.tree.branch.frontier_task_id, null);
  assert.equal(result.tree.branch.frontier_track, null);
  assert.match(result.tree.tree_answer.where_are_we, /between v11\.2 and the next concrete branch/i);
});
...
test("releaseStateAuditCommand includes product value, production boundary, and controlled candidate release gates", async () => {
  ...
  assert.equal(result.ok, true);
  assert.equal(result.summary.active_release.release_version, "11.2.0");
