...
現在の stable release は `v12.0.0` である。
...
- v9.7 は lifecycle traceability baseline evidence であり、rendered UX quality、third-party comprehension、semantic correctness、provider package availability、production provider execution safety、market adoption の証明ではないと明示できること
- Role Routing Fail-Closed Runtime により、Council実行前に必須Roleを具体Actorへ解決できること
...
- GitHub Read-Only Observation Proof により、AOFが実GitHub状態を読み取り、次の最重要タスクを日本語・根拠付き・Go/No-Go付きで選べること
- v11.0 は read-only observation proof であり、GitHub書き込み、Issue/PR作成、secret access、semantic truth、third-party adoption、productivity improvement の証明ではないと明示できること
- External Operator Reproduction Drill により、初見または外部オペレーターがAOFの次タスク判断を5分以内に理解・再現・Go/No-Go判断できる証跡パスを持てること
...
- Agent Session Contract and Tool Governance Evidence により、AI作業sessionを release-ready evidence として扱う前に、prompt / response / tool call / tool safety / approval policy / task refs / requirement refs / test refs / risk candidates / decision candidates / stop condition を監査できること
...
- Tool Governance Replay in Mission Control により、agent-session tool_call を allowed / denied / review-required / unknown / risky / external-write / dangerous としてMission Controlで確認できること
- v11.9 は recorded tool governance replay proof であり、semantic truth、model correctness、credential safety、provider behavior、production execution safety、unrecorded tool-call absence の証明ではないと明示できること
- Provider-backed Operator Decision Console により、provider-read decision、freshness、tool governance、external runtime safety、approval boundary、rollback boundary、no-autonomous-write boundary を1つのread-only Mission Control surfaceで確認できること
- v12.0 は provider-backed decision console proof であり、provider truth、semantic truth、credential safety、billing safety、market value、production execution safety、自律外部write権限の証明ではないと明示できること
...
- `release-state-audit` が `9.6.0` 以降の release で `provider-cost-quota-boundary-audit` を release gate として実行できること
- v9.6 は provider spend/quota boundary evidence であり、actual provider billing correctness、actual quota enforcement、production execution safety、provider semantic correctness、third-party adoption、market value の証明ではないと明示できること
- External Runtime Incident Drill and Recovery Evidence を release-state governance gate として扱えること
- `provider-incident-recovery-record` によって controlled execution candidate / approval / reproduction / rollback / outcome / learning / operator acceptance / production boundary を incident scenario、detection、containment、rollback decision、recovery、operator notification、learning update、resume/stop governance に束ねられること
- `provider-incident-recovery-audit` によって incident recovery refs、detection/containment boundary、rollback/recovery path、stop conditions、runtime provenance、hidden resume authority の欠落を検査できること
- `release-state-audit` が `9.5.0` 以降の release で `provider-incident-recovery-audit` を release gate として実行できること
- v9.5 は provider incident recovery drill evidence であり、live production recovery performance、actual rollback execution、provider semantic correctness、credential/billing safety、customer impact absence、third-party adoption、market value の証明ではないと明示できること
- Provider-backed Controlled Execution Candidate を release-state governance gate として扱えること
- `provider-controlled-execution-candidate-record` によって、approval / target operation / reproduction / rollback / outcome / learning / operator acceptance / product value / production boundary を 1 つの operator go/no-go candidate に束ねられること
- `provider-controlled-execution-candidate-audit` によって、candidate refs、boundary、stop conditions、runtime provenance、production execution not authorized を検査できること
- `release-state-audit` が `9.4.0` 以降の release で `provider-controlled-execution-candidate-audit` を release gate として実行できること
- v9.4 は provider-backed action を controlled candidate として説明する evidence であり、production provider execution、provider semantic correctness、credential/billing safety、actual rollback execution、market adoption の証明ではないと明示できること
- Capability-First Release Comprehension を release-state governance gate として扱えること
- `.aof/product-capabilities.json` によって、内部実装ではなく利用者が認識できる capability を ID / 説明 / 利用者価値 / 初登場Version / evidence refs として管理できること
- [Capability Matrix](docs/aof-capability-matrix.md) によって、v7 / v8 / v9 の違いを artifact 数ではなく capability 差分で説明できること
- `capability-release-delta-record` によって、NEW / UPDATED / REMOVED capability、1分価値説明、30秒版差分、value evidence、product review trigger を記録できること
- `capability-first-release-audit` によって、release notes が「What You Can Do Now / Capability Delta / Capability Matrix / Value Evidence」から始まり、README が Capability Matrix を露出しているか検査できること
- `release-state-audit` が `9.3.0` 以降の release で `capability-first-release-audit` を release gate として実行できること
- v9.3 は release value comprehension governance evidence であり、market adoption、semantic correctness、broad user satisfaction、production provider safety の証明ではないと明示できること
- External Runtime Production Boundary Candidate を release-state governance gate として扱えること
- `provider-production-boundary-record` によって、provider execution approval / reproduction / rollback / outcome / learning / operator acceptance / product value evidence を credential、budget、revocation、monitoring、incident、human go/no-go、not-proven boundary に束ねられること
- `provider-production-boundary-audit` によって、本番実行前の境界欠落、未解決ref、production execution authorizationの過大claimを検査できること
- Mission Control が provider production boundary readiness を canonical runtime artifacts から投影できること
- v9.2 は pre-production boundary evidence であり、production provider execution、provider semantic correctness、credential/billing safety、market truth、actual rollback execution の証明ではないと明示できること
...
- product capability model: [docs/aof-product-capability-model.md](docs/aof-product-capability-model.md)
- product capability matrix: [docs/aof-capability-matrix.md](docs/aof-capability-matrix.md)
...
- current release definition: [docs/v12.0-release-definition.md](docs/v12.0-release-definition.md)
- current release checklist: [docs/v12.0-release-checklist.md](docs/v12.0-release-checklist.md)
- current release notes: [docs/v12.0.0-release-notes.md](docs/v12.0.0-release-notes.md)
