...
async function loadProviderIncidentRecoveryProjection(projectRoot) {
  const auditRef = ".aof/artifacts/provider-incident-recoveries/provider-incident-recovery-audit.json";
  const audit = await maybeReadJsonByRef(projectRoot, auditRef, "provider incident recovery audit");
  if (!audit) {
    return {
      present: false,
      audit_ref: auditRef,
  ...
  return {
    ...
    audit_ref: auditRef,
    audit_ok: Boolean(audit.ok),
    recovery_count: audit.summary?.recovery_count ?? 0,
    ready_count: audit.summary?.ready_count ?? 0,
    blocked_count: audit.summary?.blocked_count ?? 0,
    resume_allowed_count: audit.summary?.resume_allowed_count ?? 0,
    hidden_resume_risk_count: audit.summary?.hidden_resume_risk_count ?? 0,
    missing_boundary_count: audit.summary?.missing_boundary_count ?? null,
    recoveries: audit.recoveries ?? [],
    not_proven: "Provider incident recovery projection proves drill governance only; it does not prove live provider recovery performance, production safety, semantic truth, or customer impact truth."
  };
}
...
async function loadProviderProductionBoundaryProjection(projectRoot) {
  ...
  if (!audit) {
    return {
      ...
      missing_boundary_count: null,
      boundaries: [],
      not_proven: "No provider production boundary audit artifact is present."
    };
  }
  return {
    present: true,
    ...
    audit_ok: Boolean(audit.ok),
    boundary_count: audit.summary?.boundary_count ?? 0,
    candidate_count: audit.summary?.candidate_count ?? 0,
    blocked_count: audit.summary?.blocked_count ?? 0,
    production_authorized_count: audit.summary?.production_authorized_count ?? 0,
    missing_boundary_count: audit.summary?.missing_boundary_count ?? null,
    boundaries: audit.boundaries ?? [],
    not_proven: "Provider production boundary projection proves pre-production eligibility boundaries only; it does not prove production execution safety, provider correctness, credential safety, billing safety, or market value."
  };
}
...
async function loadProviderReadDecisionReplayProjection(projectRoot) {
  const auditRef = ".aof/artifacts/provider-read-decision-replays/provider-read-decision-replay-audit.json";
  const audit = await maybeReadJsonByRef(projectRoot, auditRef, "provider read decision replay audit");
  if (!audit) {
    return {
      present: false,
      audit_ref: auditRef,
      ...
      latest_decision_state: null,
      latest_feedback_route: null,
      latest_summary: null,
      latest_next_action: null,
      latest_replay_ref: null,
      replays: [],
      not_proven: "No provider read decision replay audit artifact is present."
    };
  }
  const replays = audit.replays ?? [];
  const latest = replays[replays.length - 1] ?? null;
  return {
    present: true,
    audit_ref: auditRef,
    audit_ok: Boolean(audit.ok),
    replay_count: audit.summary?.replay_count ?? 0,
    accepted_count: audit.summary?.accepted_count ?? 0,
    blocked_count: audit.summary?.blocked_count ?? 0,
    reopened_count: audit.summary?.reopened_count ?? 0,
    deferred_count: audit.summary?.deferred_count ?? 0,
    needs_review_count: audit.summary?.needs_review_count ?? 0,
    latest_decision_state: latest?.decision_state ?? null,
    latest_feedback_route: latest?.feedback_route ?? null,
    latest_summary: latest?.mission_control_summary ?? latest?.operator_summary ?? null,
    latest_next_action: latest?.next_action ?? null,
    latest_replay_ref: latest?.artifact_ref ?? null,
    replays,
    not_proven: "Provider read decision replay projection proves decision/evidence linkage is visible in Mission Control; it does not prove provider truth, semantic truth, live provider availability, or external adoption."
  };
}
...
async function loadProductValueEvidenceProjection(projectRoot) {
  ...
  if (!audit) {
    return {
      ...
      records: [],
      not_proven: "No product value evidence audit artifact is present."
    };
  }
  return {
    present: true,
    ...
    audit_ok: Boolean(audit.ok),
    record_count: audit.summary?.record_count ?? 0,
    understood_count: audit.summary?.understood_count ?? 0,
    not_understood_count: audit.summary?.not_understood_count ?? 0,
    blocking_record_count: audit.summary?.blocking_record_count ?? 0,
    capability_row_count: audit.summary?.capability_row_count ?? 0,
    records: audit.records ?? [],
    not_proven: "Product value evidence projection proves a bounded capability explanation and understanding check exists; it does not prove market value, broad adoption, or semantic correctness."
  };
}
...
function buildMissionControl({
  ...
  return {
    ...
    provider_incident_recovery_projection: providerIncidentRecoveryProjection ?? {
      present: false,
      audit_ref: ".aof/artifacts/provider-incident-recoveries/provider-incident-recovery-audit.json",
      audit_ok: null,
      recovery_count: 0,
      ready_count: 0,
      blocked_count: 0,
    ...
    product_value_evidence_projection: productValueEvidenceProjection ?? {
      present: false,
      audit_ref: ".aof/artifacts/product-value-evidence/product-value-evidence-audit.json",
      audit_ok: null,
      record_count: 0,
      understood_count: 0,
      not_understood_count: 0,
    ...
    provider_read_decision_replay_projection: providerReadDecisionReplayProjection ?? {
      present: false,
      audit_ref: ".aof/artifacts/provider-read-decision-replays/provider-read-decision-replay-audit.json",
      audit_ok: null,
      replay_count: 0,
      accepted_count: 0,
      blocked_count: 0,
