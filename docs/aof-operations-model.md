...
```bash
git clone --branch v3.2.0 https://github.com/ai-org-labs/ai-organization-framework.git ~/.local/share/aof/v3.2.0
...
## Loop Definition And Stop Gate

AOF における loop は、単なる反復ではない。

```text
Loop = goal + execution + verification + stop condition
```

Verify のない反復は AOF loop ではない。
それは AI が自分の出力を繰り返すだけの自己満足反復であり、quality claim の根拠にしてはいけない。

最小の compliant loop は次を持つ。

- goal: 何を改善・検証する loop なのか
- execution: 誰が何を実行したのか
- verification: Maker とは分離された Checker が何を確認したのか
- stop condition: どの条件で stop / defer / reopen するのか

loop は必ず budget / stop gate を持つ。

- max iterations
- token or cost budget
- confidence threshold
- human review threshold
- stop / defer / reopen rule

停止条件のない loop は AOF non-compliant とする。
無制限に iteration を続けることを、学習・改善・自律性と呼んではいけない。

## Manual Proof First

AOF は、未証明の運用をいきなり自動化しない。
...
manual run
  -> skill / packet 化
  -> loop 化
  -> cadence / automation 化
```

manual run で artifact chain と human review path を再構成できないものは、まだ自動化してはいけない。
cadence / automation は、manual proof で goal、verification、stop condition、loss boundary が確認された後にだけ昇格できる。
