...
- QIF provider profile: `v0.5.4`
- active executable package baseline: v0.2.1-compatible
- QIF v0.5.4 guided elicitation is an executable external provider capability, not yet a replacement verifier inside AOF
...
- external provider baseline: Archmap `v0.4.0`; optional archmap-icons remain recognition aids only

Mission Control may project map state, but Archmap source and AOF artifacts remain the source of truth.

Provider capabilities such as lifecycle views, Requirements / Traceability / Quality projections, deterministic graph trace APIs, Runtime views, ScreenFlow / Prototype views, diagram tags, exports, or icon packs are available tools. They do not prove architecture quality unless a governed work item ties them to a Quality Intent, Archmap impact record, Council disposition, and reproducible evidence.
